...
% 1-2 → Machine Learning, 3-8 → Mechanistic Interpretability
\newcommand{\navslide}[1]{%
\begin{frame}{}
\begin{enumerate}
  % --- 1. Machine Learning ---
  ...
  \item \navparent{3}{8}{#1}{Mechanistic Interpretability}
  \begin{enumerate}
...
\begin{tikzpicture}[
  ...
  % Output tokens at the top
  \node[tok, fill=green!15] at (0, 5.6) {cat};
  \node[tok, fill=green!15] at (1.8, 5.6) {sat};
  \node[tok, fill=green!15] at (3.6, 5.6) {on};
  \node[tok, fill=green!15] at (5.4, 5.6) {the};
  \node[tok, fill=green!15] at (7.2, 5.6) {mat};
  \node[tok, fill=green!15] at (9, 5.6) {.};

  \foreach \x in {0, 1.8, 3.6, 5.4, 7.2, 9}
    \draw[arr] (\x, 5.15) -- (\x, 5.32);
...
\begin{description}
\item[Gated SAEs] Separate gating from magnitude.\textsuperscript{1}
\item[JumpReLU SAEs] Hard threshold: features fully on or off, no shrinkage.\textsuperscript{1}
\item[Top-K SAEs] Keep only the $K$ largest activations, zero the rest.\textsuperscript{2}
\item[Matryoshka SAEs] Train on multiple top-$N$; important features ranked first.\textsuperscript{3}
\item[Staircase SAEs] One SAE across layers, mirroring residual stream.\textsuperscript{4}
\item[Transcoders] Map MLP inputs to outputs directly.\textsuperscript{5}
\end{description}

\vfill
{\scriptsize\color{gray}%
\textsuperscript{1}~\cite{rajamanoharan2024}\quad
\textsuperscript{2}~\cite{bussmann2024}\quad
\textsuperscript{3}~\cite{bussmann2024a}\quad
\textsuperscript{4}~\cite{fillingham2025}\quad
\textsuperscript{5}~\cite{ameisen2025}%
}
\end{frame}
...
\begin{frame}{Attention Patterns and Logit Attribution}
\begin{center}
...
\begin{frame}{Neuronpedia: Explore Attribution Graphs}
\begin{columns}[c]
\begin{column}{0.55\textwidth}
\begin{center}
\includegraphics[width=0.6\textwidth]{figures/neuronpedia_graphs_qr.png}
\end{center}
\end{column}
\begin{column}{0.4\textwidth}
\Large
...
\vspace{1em}
\scriptsize
\texttt{neuronpedia.org/gemma-2-2b/\\graph}
\end{column}
\end{columns}
\end{frame}
